    ...
    const { data: items, error: itemsError } = await supabase
      ...
      .select(`
        *,
        items(code, name)
      `)
      .eq('sales_retail_id', req.params.id);
...
router.post('/', async (req, res) => {
  try {
    ...
    // ✅ FIX: Make created_by optional
    const created_by_uuid = req.user && req.user.id ? req.user.id : null;

    // Create sale - Only include created_by if it exists
    const saleData = {
      invoice_number,
      sale_date,
    ...
    if (created_by_uuid) {
      saleData.created_by = created_by_uuid;
    }
    ...
    for (const item of items) {
      ...
      if (currentItem) {
        const newStock = parseFloat(currentItem.stock_quantity || 0) - parseFloat(item.quantity);
    ...
    if (customer_id && (payment_status === 'unpaid' || payment_status === 'credit')) {
      ...
      if (customer) {
        const newBalance = parseFloat(customer.op_balance || 0) + parseFloat(total_amount);
    ...
    res.json({ sale, message: 'Sale created successfully' });
  } catch (error) {
